...
module.exports = function (app) {
  ...
  nunjucksEnv.addFilter('lastTopicFromCategory', function (categoryId, cb) {
    Topic.find({category: categoryId}).sort('-createdAt').exec(function (err, topics) {
      if (topics.length > 0) {
        // return topics[0].title;
        return cb(null, 3333);
      } else {
        return cb(null, 'Еще нет');
      }
    });
